...
{														    //указатель на массив целых чисел(вспомогат.) и указатели 
	...
	*posle = 0;
	...
	input[index[k-1]] = '\0';
	index = (int*)realloc(index, (*pered+1)*sizeof(int));
	input = (char*)realloc(input, strlen(input)*sizeof(char));               //После последнего разделителя ставим символ конца строки для
...
{																	   //предложений "до" и двумерный символьный массив,
	int k =0, e = 0, i;
	char* zapas = output;//который заполним отформатированными предл.
	for (k = 0; k<pered; k++){										   //Цикл заполнения двумерного массива:
		...
		for(e; e<index[k+1]; e++){									     //для данного "е" пробелом, '\t' или '\n' 
			*output++ = input[e];								     //Когда "е" примет значение какой-то буквы 
									     //данная строка двумерного массива начнет 
		}
		*output++ = '\n';			//заполнятся. Последний символ  - конец строки.
     								     //Печатаем данную строку.
	}																   //
	*output = '\0';
	output = zapas;
